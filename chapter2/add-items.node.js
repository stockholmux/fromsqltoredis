...
async.each(items, function(anItem,cb) {                     // Run the function for each item in the array
  ...
  client                                                    // refer to the client object which represents a single connection
    ...
    .sort(                                                  // SORT command
      rk(keyRoot,'priceIndex'),                             // on the key "redishop:priceIndex"
      'BY', 'nosort',                                       // no sorting, just foreign key
      'GET', itemsKey+'->manufacturer',                     // 'GET' then 'redishop:items:*->manufacturer' to get the manufacturer 
      'STORE',                                              // STORE the results of the sort command
      rk(                          
        keyRoot,
        'sorted',
        'items-manufacturer',
        'price'                                             // At "redishop:sorted:items-manufacturer:price" (a list)
      )
    )
